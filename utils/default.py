...
def create_model(args):
    if 'wideresnet' in args.arch:
        ...
        model = models.resnet18(num_classes=args.num_classes, open=(args.model == 'open_match'))
    else:
        raise ValueError(f"Not supported architecture: {args.arch}")
    return model
...
def set_model_config(args):
    if args.dataset == 'cifar10':
        ...
        args.label_classes = 60

    elif args.dataset == "imagenet":
        args.label_classes = 20
        if 'oracle_' in args.model:
            args.num_classes = 30
        args.label_classes = 20

    if 'os_' in args.model:
        args.num_classes = args.label_classes + 1
    elif 'oracle_' not in args.model:
        args.num_classes = args.label_classes

    if args.model in ['dummy_match', 'fix_contrast', 'fix_match_uniform']:
        args.num_classes = args.label_classes + args.dummy_classes
...
def set_dataset(args):
    train_labeled_dataset, unlabeled_dataset, test_dataset, val_dataset, labeled_dataset = \
    ...
    return train_labeled_trainloader, unlabeled_dataset, \
           test_loader, val_loader, ood_loaders, labeled_loader
